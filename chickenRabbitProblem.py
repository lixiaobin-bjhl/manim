...
class ChickenRabbitProblem(Scene):
    def construct(self):
        ...
        # STEP 1：题目展示（顶部）
        title = Text("鸡兔同笼", font_size=48, color=BLUE).to_edge(UP, buff=0.5)
        ...
        leg_text = Text(f"总脚数：{initial_legs}", font_size=32)
        ...
        diff_text = Text(f"实际脚数差：28 - 20 = {diff_initial}", font_size=32, color=RED)
        ...
        step1 = MathTex(f"{total_legs_target}", "-", f"{initial_legs}", "=", f"{diff_initial}",
                        font_size=30)
        step1.to_corner(DR, buff=0.5)
        self.play(Write(step1))
        self.wait(1)
        
        # 展示每换一只增加的脚数
        delta_text = MathTex("\\text{每换一只兔多}", f"{delta_per}", "\\text{只脚}",
                           font_size=30)
        delta_text.next_to(step1, UP, aligned_edge=RIGHT, buff=0.3)
        ...
        replace_text = MathTex(r"\text{需要将}", f"{replacements}", r"\text{只鸡换成兔}",
                              font_size=30)
        replace_text.next_to(step2, DOWN, aligned_edge=RIGHT, buff=0.3)
        ...
        def update_leg_diff():
            ...
            new_text = Text(f"当前总脚数：{int(current)}", font_size=30)
        ...
        # 在第四只兔子上打勾
        check = Text("✔", font_size=64, color=GREEN)
